...
def cosine_similarity(A, B):
    A = F.normalize(A, dim=1)  # 对A进行L2归一化
    B = F.normalize(B, dim=1)  # 对B进行L2归一化
...
def run():
    ...
    model = VAE(input_dim=400, hidden_dim=hidden_dim, latent_dim=latent_dim)
