...
def run():
    ...
    optimizer = optim.RMSprop(list(model.parameters())+list(mlp.parameters()), lr=1e-2)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            train_fea = mlp(traindata_)
            test_fea = mlp(testdata_)
        ...
        print('epoch:{:d}, rec_loss:{:.6f}, nce_loss:{:.6f}, center_loss:{:.6f}'.format(epoch,reconstruction_loss,loss_nce,center_loss))
    print("best acc is ", max_acc)
    # plt.plot(accs)
    # plt.title('my')
    # plt.xlabel('Epochs')
    # plt.ylabel('Accuracy')
